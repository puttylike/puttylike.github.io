...
<ol>
<li><strong>개념 및 동작원리</strong><br>
편미분을 체인 룰 chain rule로 국소 local 미분으로 분리해서 수학 공식으로 표현해 계산하는 방법</li>
</ol>
...
<li>$W^2$가 변하면 $A^2$가 변한다 => $∂A^2/∂W^2$
​
=> 미분을 어떤 공식의 곱하기들로 표현하면, 속도가 빨라지지 않을까?</li>
</ul>
</li>
</ul>
...
<li>$E = ∑ Ei$ (MSE)<br>
=> $∂E/∂W$ 이런 거로 $A*B*C*D$ 이런 형태로 표현할 거다.</li>
</ul>
</li>
<li>
<p>각 층의 선형회귀 값 (z) / 각층의 출력 값 (a) 구하기<br>
: 각각의 층에서 선형회귀 값 z과 출력값 a을 구한다</p>
<ul>
<li>$E = E1 + E2$ , $E1$에 영향을 주는 $a_1^3$과 $E_2$는 별개니까</li>
<li>입력층에는 가중치가 적용되지 않는다.<br>
입력층 출력 값 => $a_1^1 = x_1$, $a_2^1 = x_2$</li>
<li>은닉층 선형 회귀 값 => $z_1^2 = a_1^1*w_11^2 + a_2^1*w_12^2 + b_1^2$,
$z_2^2 = a_1^1*w_21^2 + a_2^1*w_22^2 + b_2^2$</li>
<li>은닉층 출력 값 => $a_1^2 = sigmoid(z_1^2)$, $a_2^2 = sigmoid(z_2^2)$
​</li>
</ul>
</li>
</ul>
...
<p>$∂E/∂w_11^2 =$
$(a_1^3-t_1^3)*a_1^3*(1-a_1^3)*w_11^3*a_1^2*(1-a_1^2)*a_1^1$
$+ (a_2^3-t_2^3)*a_2^3*(1-a_2^3)*w_21^3*a_1^2*(1-a_1^2)*a_1^1$</p>
<p>은닉층 오차역전파 가중치 공식 : 다음층 $(a_1^3-t_1^3)*a_1^3*(1-a_1^3)$ | $w_11^3*a_1^2*(1-a_1^2)$ | 이전층 $a_1^1$  => 가운데만 알면 된다.</p>
...
<p>오차역전파 공식 구하는 순서</p>
<ul>
<li>출력층의 손실 = (출력층출력 - 정답) X 출력층출력(1 - 출력층출력)<br>
=> $loss_3 = (A^3 - Target) * A^3 * (1-A^3)$</li>
<li>현재층에 적용되는 가중치 변화율 ∂E/∂W = (이전층 출력)^T• 현재층 손실<br>
=> $W_3 = W_3 - α∂E/∂W_3 = W_3 - α*(A^2^T•loss_3)$</li>
<li>현재층의 바이어스 변화율 ∂E/∂b = 현재층 손실<br>
=> $b_3 = b_3 - α∂E/∂b_3 = b_3 - α*loss_3)$</li>
</ul>
</li>
</ul>
